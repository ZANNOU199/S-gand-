      ...
      <section className="py-16 md:py-24 px-6 md:px-8 max-w-[1440px] mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-10 md:mb-12">
          <div>
            ...
            <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight">Nos Univers</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {sectors.map((sector) => (
            ...
            >
              <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-110 opacity-70 group-hover:opacity-100" style={{ backgroundImage: `url('${sector.image || "https://via.placeholder.com/800x1200"}')` }}></div>
      ...
      <section className="bg-background-dark py-16 md:py-24 border-t border-white/5">
        <div className="max-w-[1440px] mx-auto px-6 md:px-8">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 md:mb-16 gap-6 text-center md:text-left">
            <div>
              <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight mb-4 leading-none text-white">Collection Sahel</h2>
              <p className="text-sand/40 text-xs md:text-sm max-w-xl uppercase font-bold tracking-widest leading-relaxed">
                Une sélection de pièces d'exception choisies par nos curateurs.
              </p>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden bg-terracotta/20 py-16 md:py-24">
        <div className="max-w-[1440px] mx-auto px-6 md:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-24 items-center">
          ...
          <div className="space-y-6 md:space-y-10 order-1 lg:order-2">
            ...
            <h2 className="text-4xl md:text-6xl font-black leading-[1.1] md:leading-[0.9] uppercase tracking-tighter text-white">Racines Ancestrales,<br/>Vision Moderne</h2>
            <button onClick={() => navigate('/journal')} className="flex items-center gap-4 text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] text-white hover:text-primary transition-all group">
              DÉCOUVRIR NOTRE HISTOIRE <ArrowRight size={18} className="group-hover:translate-x-2 transition-transform" />
            </button>
          </div>
        </div>
      </section>
