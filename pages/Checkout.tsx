    ...
    <div className="bg-background-dark min-h-screen py-24 text-white">
      <div className="max-w-6xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          {step === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right duration-500">
              ...
              <div className="p-6 border border-primary bg-primary/5 rounded-xl flex items-center gap-4">
                ...
                <span className="font-bold uppercase tracking-widest text-sm">Carte Bancaire / Mobile Money</span>
              </div>
        ...
        <div className="bg-charcoal/50 p-8 rounded-2xl border border-white/10 h-fit">
          <h3 className="font-black uppercase tracking-widest mb-8 text-sm">Votre Panier</h3>
            ...
            {cart.map(item => (
              <div key={item.id} className="flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[9px] text-sand/40">Qté: {item.quantity}</p>
                </div>
                <p className="text-[10px] font-bold text-primary">{(item.price * item.quantity).toLocaleString()} FCFA</p>
              </div>
            ))}
          </div>
          <div className="border-t border-white/10 pt-4 space-y-2">
            <div className="flex justify-between text-[10px] text-sand/50"><span>Sous-total</span><span>{total.toLocaleString()} FCFA</span></div>
            <div className="flex justify-between text-[10px] text-sand/50"><span>Livraison</span><span>Gratuit</span></div>
            <div className="flex justify-between text-base font-black text-white pt-2"><span>TOTAL</span><span>{total.toLocaleString()} FCFA</span></div>
          </div>
        </div>
      </div>
    </div>
